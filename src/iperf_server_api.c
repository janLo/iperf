...
{
    char * addr = (test->control_address == NULL ? test->bind_address : test->control_address);
    retry:
    if((test->listener = netannounce(test->settings->domain, Ptcp, addr, test->server_port)) < 0) {
